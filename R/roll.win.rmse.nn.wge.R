...
roll.win.rmse.nn.wge = function(series, horizon = 1, s = 0, d = 0, fit_model)
{
  ...
  lags = fit_model$lags
  trainingSize = max(lags,s,d,9) #Long story but forecast::ets does not like series smaller than 8 unit and nnfor calls "trendcheck" when difforder = 0 which returns error for lenth < 8.
