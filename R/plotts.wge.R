plotts.wge = function (x,style = 0,ylab = "",xlab = "Time", main = "", col = "black", text_size= 12, lwd = .75, cex = .5, cex.lab = .75, cex.axis = .75, xlim = NULL, ylim = NULL) 
{
  #
    ...
    if (class(x) != "ts") {
      if (n <= 200) {
        plot(t, x, type = "o", cex = cex, pch = 16, cex.lab = cex.lab, 
             ...
             ylab = ylab, col = col, main = main, las = 1,xlim = xlim, ylim = ylim, las = 1)
      }
      else if (n > 200) {
    ...
    if (class(x) == "ts") {
      if (n <= 200) {
        plot(x, type = "o", cex = cex, pch = 16, cex.lab = cex.lab, 
             cex.axis = cex.axis, lwd = lwd, xlab = xlab, 
             ylab = ylab, col = col, main = main,xlim = xlim, ylim = ylim, las = 1)
      }
      else if (n > 200) {
